...
- **Auto-update**: built on GitHub Releases; downloads are routed through the gateway itself when running (enjoying upstream failover), with triple integrity checks and atomic replacement.
...
- **Prebuilt binaries**: grab `proxyone.exe` (or the Linux tarball) from the [Releases](https://github.com/zhuchentong/proxy-one/releases) page. Launching it opens the GUI and starts proxying immediately.
- **Build from source**:
...
The settings-page switch writes, per user and without administrator rights, the registry Run key on Windows (`HKCU\...\Run\proxyone`) or an XDG autostart desktop entry on Linux (`~/.config/autostart/proxyone.desktop`). At login the app starts with `--minimized` and hidden, so the proxy is ready immediately. If the executable moves or the app is renamed, the saved path is repaired on the next start (legacy-name leftovers are cleaned up and the autostart intent migrates automatically). The switch state follows the system: the GUI reads the real state at every launch.
...
- **Atomic replacement**: uses "Windows allows renaming a running exe" to perform the swap `current exe → .old`, `.new → current name`, then detaches the new version and exits; the system proxy is restored first, and the new instance re-takes-over per the persisted intent.
...
- User config directory: `%LOCALAPPDATA%\proxyone` on Windows, `$XDG_CONFIG_HOME/proxyone` (default `~/.config/proxyone`) on Linux. Data directory (state, logs): the same path on Windows, `$XDG_DATA_HOME/proxyone` (default `~/.local/share/proxyone`) on Linux.
...
- **Protocol detection**: with `type = "auto"`, a SOCKS5 handshake is sent to the upstream; a `0x05` response marks SOCKS5, anything else HTTP. Detection results are cached, or the type can be fixed manually.
- **Active health checks**: every `interval_secs` a GET to `test_url` goes through each upstream (expecting 2xx) with latency recorded; `fail_threshold` consecutive failures mark DOWN, `success_threshold` consecutive successes recover UP; a round runs immediately at startup.
- **Passive fast failover**: a failed dial on a new connection (refused / handshake error) marks the upstream DOWN immediately and switches in-place before any byte is forwarded, also triggering a full check round; a target-side failure discovered after the tunnel is established (non-2xx CONNECT, SOCKS5 error code) does not mark DOWN but still switches.
- **Routing and switch-back**: every new connection picks the highest-priority UP upstream; recovered high-priority upstreams regain new connections automatically. If everything is DOWN, dialing still proceeds by priority.
- **No connection migration**: switching affects new connections only; established connections keep their upstream until they end.
...
- Inbound SOCKS5 supports no-auth only (the listener binds loopback only, so local use needs no auth)
...
- Switching affects new connections only; inbound SOCKS4 requests are rejected (SOCKS5 / HTTP only)
